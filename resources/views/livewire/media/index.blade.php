<div>
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-2">Manage your uploaded media files from various sources.</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
        ...
        @foreach($this->stats['by_type'] as $type => $count)
            @if($count > 0)
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow dark:shadow-gray-700">
                    <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $count }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400 capitalize">{{ $type }}s</div>
                </div>
            @endif
        @endforeach
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow dark:shadow-gray-700">
            <div class="text-lg font-bold text-purple-600 dark:text-purple-400">{{ $this->formatBytes($this->stats['total_size']) }}</div>
    ...
    @if($this->media->count() > 0)
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($this->media as $mediaItem)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-700 hover:shadow-lg dark:hover:shadow-gray-600 transition-shadow duration-200">
                    {{-- Media Preview --}}
                    <div class="aspect-square bg-gray-100 dark:bg-gray-700 rounded-t-lg flex items-center justify-center overflow-hidden">
                        ...
                        @else
                            <div class="text-center p-4">
                                <x-mary-icon :name="$mediaItem->media_type->icon()" class="w-12 h-12 text-gray-400 dark:text-gray-500 mx-auto mb-2" />
                                <div class="text-xs text-gray-500 dark:text-gray-400 uppercase font-medium">{{ $mediaItem->media_type->label() }}</div>
                            </div>
                        @endif
                    </div>
                    ...
                    {{-- Media Info --}}
                    <div class="p-3">
                        <div class="font-medium text-sm text-gray-900 dark:text-gray-100 truncate" title="{{ $mediaItem->original_name }}">
    ...
    @else
        ...
        <div class="text-center py-12">
            <x-mary-icon name="o-photo" class="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No media files found</h3>
